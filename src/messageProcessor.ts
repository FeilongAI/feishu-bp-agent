...
export class MessageProcessor {
  ...
  async process(message: IncomingMessage): Promise<BotReply> {
    ...
    const enrichedMessage = this.senderDirectory
      ...
      : message;
    const key = conversationKey(enrichedMessage);
    try {
      return await this.store.withConversationLock(key, async (lockedStore) => {
        const claim = await lockedStore.claimMessage(enrichedMessage.messageId, key);
        if (!claim.claimed) {
      ...
      await this.store.failMessage(enrichedMessage.messageId, "processing_failed").catch(() => undefined);
      this.logger.error("message_processing_failed", { messageId: enrichedMessage.messageId, senderId: enrichedMessage.senderId, error });
      throw error;
    }
  }
}
