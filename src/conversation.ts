...
export class ConversationService {
  ...
  handleMessage(message: IncomingMessage): BotReply {
    if (message.senderType === "bot") return { text: "" };

    const key = `${message.chatId}:${message.senderId}:${message.threadId ?? "main"}`;
    const conversation = this.store.getConversation(key) ?? this.newConversation(message, key);
    ...
    if (this.isCurrentWorkQuery(text)) {
      ...
      return { text: this.currentWorkReply() };
    }
    if (this.isMyRequirementsQuery(text)) {
      this.store.saveConversation(conversation);
      return { text: this.myRequirementsReply(message.senderId) };
    }
  ...
  private currentWorkReply(): string {
    const active = this.store.listRequirements({ status: "进行中" }).filter((item) => item.ownerId === this.config.ownerId);
  ...
  private myRequirementsReply(requesterId: string): string {
    const items = this.store.listRequirements({ requesterId });
